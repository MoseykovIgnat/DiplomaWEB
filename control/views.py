...
def get_more_alert_history_info(request):
    if request.method == 'GET' and request.is_ajax():
        ...
        new = ScAlertHistory.objects.filter(
            ((Q(creator=user) & Q(is_required_condition=0)) | Q(is_required_condition=1)) & Q(id__gt=request.POST.get('last_alert_id'))).order_by('-time_calc').values()
        print(new)
        ...
        for element in new_alerts_to_play:
            try:
                ...
                if alert["priority"] > most_primary_alert["priority"]:
                    most_primary_alert["priority"] = alert["priority"]
